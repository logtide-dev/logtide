...
- **Bumped `js-yaml` to 4.3.x** (Dependabot `GHSA-52cp-r559-cp3m`, HIGH): versions `>=4.0.0 <4.3.0` expand chained YAML merge keys (`<<`) in quadratic time, so a small document can consume minutes of CPU. `js-yaml` is a direct backend dependency and `yaml.load()` runs on caller-supplied documents in three import endpoints (Sigma rules, custom dashboards, log pipelines), so an authenticated user could stall a worker with a short payload. New root pnpm override pinned to the 4.x line (`>=4.3.0 <5`; 5.x is a major with a changed API).
- **Bumped `undici` to 7.29.0** (Dependabot `GHSA-4cwx-7wf7-3272` HIGH, plus `GHSA-m8rv-5g2x-5cg5`, `GHSA-v3r7-h72x-cjcm`, `GHSA-jr45-8vmc-qm54`, `GHSA-8xcm-r25x-g524`): a set of response-parsing and interceptor issues (cross-user disclosure via degenerate or whitespace-padded cache directives, CRLF injection through a blob body `type`, cookie attribute injection, and response desynchronization in the retry interceptor). LogTide uses `undici` in exactly one place, the SSRF guard's outbound fetch (`src/utils/ssrf-guard.ts`), with a plain `Agent` and no cache or retry interceptor, and sends string bodies without cookie handling, so none of the five were reachable as configured. The override floor was raised from `>=7.28.0 <8` to `>=7.29.0 <8`; the `<8` bound is intentional.
- **Bumped `find-my-way` to 9.7.0** (Dependabot `GHSA-c96f-x56v-gq3h`, HIGH): Fastify's router can be driven into a denial of service over HTTP/2. LogTide serves HTTP/1.1 everywhere (no `http2` option is set on the Fastify instance and no deployment config enables it), so this was not reachable, but the router is on the request path for every API call and the floor is worth holding. New root pnpm override.
- **Bumped `fast-uri` to 3.1.5** (Dependabot `GHSA-7p8r-x3mc-p8w7`, `GHSA-v2hh-gcrm-f6hx`, `GHSA-4c8g-83qw-93j6`, all HIGH): host confusion when parsing authorities that use a backslash introducer or that fail IDN canonicalization, i.e. the parser and a consumer can disagree about which host a URL points at. It reaches LogTide only through `ajv`'s `format: 'uri'`, used on the `webhookUrl` field of two Sigma routes. Nothing security-relevant is decided from that validation: outbound requests are gated by the SSRF guard, which re-parses the URL with Node's `URL`, resolves it with `dns.lookup` and revalidates every redirect hop. The override floor was raised from `>=3.1.2` to `>=3.1.5 <4` (4.x is a major; the `<4` bound keeps Fastify's expectations intact).
- **Bumped `postcss` to 8.5.26** (Dependabot `GHSA-r28c-9q8g-f849`, `GHSA-6g55-p6wh-862q`, both HIGH): an attacker-controlled `sourceMappingURL` comment in CSS makes the previous-source-map auto-loader read an arbitrary `.map` file off disk and fold its contents into the build output. PostCSS runs only at build time here (Tailwind via Vite) over first-party stylesheets, so exploiting it would require already controlling the repository's CSS sources. New root pnpm override.
- **Bumped `brace-expansion` to 5.0.9** (Dependabot `GHSA-rgw5-rvv9-x895` and `GHSA-mh99-v99m-4gvg`, HIGH): the first is an explicit bypass of the `CVE-2026-13149` mitigation this project already pinned for in 1.2.0 (unbounded intermediate arrays), the second an out-of-memory crash from unbounded expansion length. Pulled in transitively through glob/minimatch in tooling, so not on any request path. The override floor was raised from `>=5.0.7` to `>=5.0.9`.
- **Bumped `shell-quote` to 1.10.0** (Dependabot `GHSA-395f-4hp3-45gv`, HIGH): `parse()` is quadratic in the input length (CWE-407). Used only by development tooling and never invoked on user input. The override floor was raised from `>=1.8.4` to `>=1.9.0`.

### Added
- **Log table panel for custom dashboards** (#289): a new `log_table` panel type shows individual log rows (newest first) with user-configured metadata columns resolved with the same semantics as Log Search custom columns (exact key first, then dot path into nested objects, e.g. `http_host` or `geo.city`), so access-log style hit lists (host, client IP, status, GeoIP city) can live directly on a dashboard. Because the panel reads raw rows instead of aggregating, nested paths work on all three storage engines. Built-in time/level/service/message columns are individually toggleable, rows are capped (10-100) and filtered by project, service and levels, cells are single-line with ellipsis and a full-value tooltip by default with an optional wrap mode (the Search table's fixed truncation was the pain point that motivated this), and clicking a row opens the log's context in Search. Two refresh modes: snapshot (dashboard auto-refresh) or live, which tails the project's WebSocket log stream (stream-ticket auth, so the session token never appears in the URL) and filters/resolves columns client-side; live mode requires a specific project and degrades to a visible reconnecting state on connection loss. The metadata path resolver moved to `@logtide/shared` so Search, the panel and the backend fetcher share one implementation. Requested by @SemoTech.
- **Geo map panel for custom dashboards** (discussion #287): a new `geo_map` panel type plots GeoIP-enriched log volume on a world map, with two modes: aggregate bubbles per country (centroid-placed, using a static country centroid table) or per rounded location (~1 km precision) with city-level tooltips. To make this aggregatable on all three storage engines, the `geoip` pipeline step now writes flat metadata keys next to its existing nested object (`<target>_country`, `<target>_country_code`, `<target>_city`, and `<target>_place`, which packs rounded coordinates plus a display label into one value so the map is a single group-by, never a raw-row scan); existing nested output is unchanged and logs ingested before this change simply do not appear on the map. The map renders with Leaflet over a bundled Natural Earth world outline (no tile servers, no CDN assets: works air-gapped and follows the app theme), bubbles scale with the square root of the count, and values that fail defensive parsing (geo metadata is client-written and untrusted) are dropped and surfaced as a skipped-values note on the panel. Filters: time window, log levels, service, hostname; the geo field prefix is configurable for pipelines with multiple GeoIP steps and is validated against a strict charset at the API boundary and again in the fetcher, since it participates in the metadata field name passed to the storage engines.

### Fixed
- **Repeated webhook events no longer get stuck at `pending` forever**: the dispatcher deduplicated double-enqueues through the queue job key, derived from `(organization, event type, event id)`. On the BullMQ backend a job id stays in Redis after the job completes unless `removeOnComplete` is set, so that key deduplicated not just concurrent enqueues but every future occurrence of the same event. Callers that reuse an event id across occurrences (monitor status changes key on monitor, status and URL) therefore delivered only the first ever occurrence: each later one still inserted a delivery row, whose job the queue silently dropped, leaving it visible in the delivery panel at `pending` with `0/5` attempts forever. The graphile backend frees the key when the job ends, so the same code behaved differently per backend. Deduplication is now explicit and bounded in the dispatcher: an identical event still in flight and created within `WEBHOOK_DEDUP_WINDOW_MS` (default 60s) reuses that delivery, and anything older or already terminal starts a new one. Queue job keys are now per delivery row, unique by construction, so the queue can only collapse two workers racing the same row, and all webhook jobs set `removeOnComplete`/`removeOnFail` so a delivery can also be replayed more than once. New index on `webhook_deliveries(organization_id, event_type, event_id, created_at DESC)` (migration 056) for the lookup. Delivery rows already stuck at `pending` stay unprocessable and can be deleted; no new ones will accumulate.
- **Webhook channels pointing at Discord now work**: Discord's webhook endpoint accepts only bodies carrying `content`, `embeds` or file parts, so it rejected the LogTide event envelope with HTTP 400. The "Test channel" button surfaced that as `HTTP 400 Bad Request` and queued events went straight to the dead-letter queue after one non-retryable attempt. Outbound bodies are now formatted per destination in the single dispatch primitive every webhook passes through, so a Discord URL receives one embed per event (title, description, severity color with green for monitor recoveries, per-type fields, organization footer, event link) for every source: channel test, monitors, alerts, incidents, errors, plus retries and replays. Detection covers `discord.com` and `discordapp.com` including `ptb.`/`canary.` subdomains and versioned API paths, and deliberately skips the `/slack` and `/github` compatibility endpoints, which expect their own payload shape. Existing channels need no reconfiguration; the delivery log keeps storing the canonical envelope so replays and the delivery detail view still show the event. The channel form flags a Discord URL as it is typed.
- **Dialogs taller than the viewport no longer hide their own buttons**: `Dialog.Content` is fixed and vertically centred with no height bound, so a dialog whose content exceeded the window spilled off both edges at once, taking the title with it upwards and the action buttons downwards, with no way to scroll to either (the dialog is fixed, so page scrolling does not reach it). Measured on a 1366x768 laptop viewport, the add-panel dialog rendered 938px tall and its "Add panel" button sat 20px below the fold, and the panel config dialog for the new log table panel rendered 1228px tall with "Save" completely out of reach. The dialog primitive now caps at `calc(100dvh - 2rem)` and scrolls as a safety net for every dialog in the app, and the two dashboard dialogs that grow with content (the panel-type list, which gains an entry with every new panel type, and the panel config form) scroll their own body so the footer actions stay pinned and visible. The add-panel dialog was already overflowing by 68px before the log table panel was added; that panel widened the overflow to 170px and pushed the primary action off-screen.
- **Settings -> Organization no longer shows a stale retention value** (#288): the organization object cached in localStorage (kept to avoid a name flash on reload) was never re-fetched once present, so a retention change made in Admin never reached the member-facing Settings page within a persisted session, and hard refreshes did not help. The dashboard now revalidates the organization list in the background on load whenever a cached organization exists (the cache still renders immediately, it just stops being authoritative), and the retention card no longer coerces a missing `retentionDays` to "90 days" via a `|| 90` fallback, which made any stale or pre-retention cached object look like a real 90 day policy; a missing value now renders as a placeholder until the refresh lands. Regression tests added for both the background revalidation and the fallback. Reported by @SemoTech with an accurate frontend root-cause analysis.
- **graphile-worker queue pool no longer crashes the backend on connection loss** (#291): the dedicated `pg.Pool` created for the PostgreSQL queue backend registered no `error` handler, so when the database terminated a pooled connection (routine restart, maintenance, host reboot, `pg_terminate_backend`), the pool's unhandled `'error'` event took down the whole backend/worker process with an uncaught exception (Node's default for unhandled EventEmitter errors). This only affected deployments running the queue on PostgreSQL (no `REDIS_URL`); the main database pool and the LISTEN/NOTIFY client already handled this correctly, the queue pool was the one gap. The pool now logs the error and lets `pg` replace the dead connection, matching the existing pattern in `connection.ts`. Regression test added. Reported by @Erasure5959 with a full production stack trace and root-cause analysis.
- **Syslog collection requirements documented** (discussion #292 follow-up): nothing stated that the bundled Fluent Bit container, which is the only syslog listener (port 514 UDP/TCP, RFC 3164), sits behind the `logging` compose profile and therefore does not run in a default installation; `install.sh` starts the stack without profiles, so users pointing network devices (Cisco, H3C) at port 514 got silence with no hint why. The README's profile list, both example env files, and the `install.sh` closing summary now spell out the two-step enablement (set `FLUENT_BIT_API_KEY`, start with `--profile logging`); `SYSLOG_TZ_OFFSET` is now documented in the env examples too, since RFC 3164 timestamps carry no timezone and devices sending local time otherwise land outside the UI's default 24h window.
...
- **Monitoring page used unstyled native `<select>` dropdowns** (UX audit): the project picker, New Monitor project/type selectors, incident severity/status selectors, and the status-page visibility selector all rendered the browser's native `<select>`, which looked out of place next to the custom dropdowns used everywhere else in the app. All six now use the shared `Select` component, so styling, focus, and open/close behavior match the rest of LogTide.
...
- **Projects page browser tab title said "Dashboard"** (UX audit): the `/dashboard/projects` list page hardcoded `<title>Dashboard - LogTide</title>`, so the browser tab and history entry read "Dashboard" instead of "Projects". Now titled "Projects - LogTide", matching every sibling page.
